...
class AdminDashboard extends Page
{
    ...
    {
        return [
            'ancaman' => LaporATHG::where('jenis_athg', 'ancaman')->count(),
            'tantangan' => LaporATHG::where('jenis_athg', 'tantangan')->count(),
            'hambatan' => LaporATHG::where('jenis_athg', 'hambatan')->count(),
            'gangguan' => LaporATHG::where('jenis_athg', 'gangguan')->count(),
        ];
    }
    ...
    private function getPerformanceMetrics(): array
    {
        ...
        $avgProcessingTime = DB::table('status_layanan')
            ->where('status', 'selesai')
            ->whereNotNull('updated_at')
            ->avg(DB::raw('DATEDIFF(updated_at, created_at)'));

        // Calculate approval rate
        $totalProcessed = DB::table('status_layanan')
            ->whereIn('status', ['selesai', 'ditolak'])
            ->count();
        
        $totalApproved = DB::table('status_layanan')
            ->where('status', 'selesai')
            ->count();
    ...
    {
        return DB::table('status_layanan')
